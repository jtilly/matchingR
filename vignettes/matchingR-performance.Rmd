...
```{r, echo=FALSE}
table = matrix(c(rep(100, 19), solution_exists, solution_exists/50, irving_original_prop[1:19], summary(res_irving)$mean),  
               ncol = 5, nrow = 19, 
               dimnames = list("N" = gsub(" ", "", sprintf("N = %s", format(N, big.mark = ","))), 
                               "t" = c("No. of instances", 
                                       "No. with solution", 
...
```{r, echo=FALSE}
table = matrix(c(rep(1, 10), summary(res_irving)$mean),  
               ncol = 2, nrow = 10, 
               dimnames = list("N" = gsub(" ", "", sprintf("N = %s", format(N, big.mark = ","))), 
                               "t" = c("No. of instances", 
                                       "Average cpu time")))
