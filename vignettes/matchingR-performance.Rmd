...
test_roommate = function(n) {
    roommate.matching(utils = matrix(rnorm( 4 * n ^ 2 ), nrow = 2 * n))
}
```
...
for (n in 1:19) {
    for (i in 1:50) {
        results = roommate.matching(utils = matrix(rnorm( 4 * N[n] ^ 2 ), nrow = 2 * N[n]))
        if (!is.null(results)) {
...
test_toptrading = function(n) {
    toptrading.matching(utils = matrix(rnorm( n ^ 2 ), nrow = n))
}
```
