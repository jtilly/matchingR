...
# Gale-Shapley Algorithm [@gale1962college]
The computational performance highly depends on how similar agents' preferences are. When some woman is the most preferred to many men, the algorithm will require a lot more rounds to compute the stable matching than when preferences are completely random. To capture this we will construct preferences that feature a common and an idiosyncratic component. The weight on the common component ("the level of commonality") is denoted by $\lambda$. When $\lambda=1$, all men have identical preferences over all women and vice versa. When $\lambda=0$, all preferences are completely random.
...
# Irving's Algorithm [@irving1985roommates]

Existence of a stable matching is not guaranteed, and indeed for large markets, can be very likely to not exist. Therefore, there are two relevant measures of speed to be considered: First, the average time to find a stable matching or to verify that no stable matching exists, and second, the average time to find a stable matching, conditional on such a matching existing. 
...
# Top Trading Cycle Algorithm [@shapley1973cores]
