...
We first need to introduce notation to specify men's and women's preferences. Men's payoffs from being matched are given by the payoff matrix `uM`. Rows in the matrix correspond to men, columns to women. For example: 
```{r}
...
uW
```
In this example, woman `1` receives a payoff of `0.0` from being matched to man `1`, a payoff of `0.5` from being matched to man `2` and a payoff of `1.0` from being matched to man 3 (same logic applies to women `2` and `3`).
...
matching$proposals
```
The first element in the vector tells us that man `1` is matched with woman `1`. Man `2` is matched to woman `3`, and man `3` is matched to woman `2`. The vector of engagement contains 
```{r, echo=FALSE}
...
```
The first element in the vector tells us that woman `1` is matched to man `1`, woman `2` will be matched to man `3`, and woman `3` will be matched to man `2`.
